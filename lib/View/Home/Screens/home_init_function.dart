...
class HomeInitFunction {
  static Future<void> initFunctionHome({required BuildContext context}) async {
    ...
    if (token != "") {
      ...
      BlocProvider.of<UserStatusCubit>(context).updateAppUserStatus(userStatus);
    } else {
  ...
  static Future<void> refreshAppDetailsFunction(

      {required BuildContext context}) async {
    final data = await HttpRequests.appDetialsRequest(
      context: context,
    );

    BlocProvider.of<AppDetailsCubit>(context).updateAppDetails(data);
  }
}
